...
const ReportCommerce = () => {
	...
	const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		Api.reportCommerce(id, newReport)
		.then(() => {
			setSuccess(true)
	...
	return (
		<div className={styles.reportCommerce}>
			{success &&
				<div className={styles.success}>
					...
					<motion.button initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1, transition: { delay: 0.8 } }} onClick={() => navigate('/')}>Volver al comercio ({successCounter})</motion.button>
				</div>
			}
